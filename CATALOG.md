...
> This project uses AI coding assistants (Claude Code, Gemini CLI, Copilot, Cursor) for Unity development. Skills are markdown files that teach the agent domain-specific workflows. Place them in `~/.gemini/skills/` (Gemini CLI) or `.claude/skills/` (Claude Code).
...
*Last updated: March 2026 · Maintained by the Antigravity Unity Skills Framework project.*